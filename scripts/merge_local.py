...
def gen_srt(text, duration, srt_path):
    """按句生成字幕，每句停留时长 ∝ 该句字数（贴近 edge-tts 逐字朗读节奏，音画同步）。

    之前是句数均匀切分（长句短句同样时长），与配音脱节；改为字数比例后，
    字幕出现/消失节奏 ≈ 配音念到该句的时刻。
    """
...
def main():
    ...
    # 最终时长 = 原视频与配音的较大者（配音短则补静音+延长画面，长则顺延/截断）
    ...
    # 视频延长到 final_dur（末帧冻结），保证配音放完时画面也刚好结束（音画同步兜底）
    vf = f"{vf},tpad=stop_mode=clone:stop_duration={final_dur:.3f}"
