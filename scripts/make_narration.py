...
# —— 文本编码自动探测（兼容中文 Windows 的 GBK 文案）——
try:
...
def main():
    ...
    # 读配音时长（供"画面适配配音"做音画同步；ffprobe 与 ffmpeg 同目录）
    try:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        ...
        if fp_bin and os.path.isfile(os.path.join(fp_bin, "ffprobe.exe")):
            ...
            if dur:
                print(f"AUDIO_DURATION={float(dur):.3f}", flush=True)
                print(f"     ↑ 画面视频将按此时长适配，保证音画同步", flush=True)
    except Exception as e:
        print(f"[warn] 读取配音时长失败（不影响配音生成）：{e}", flush=True)
